<template>
  <div class="group-wrapper">
    <h2 class="group-title">Monitor: {{ groupName }}</h2>
...
const groupName = ref(route.params.monitorType);

console.log("groupName");
console.log(groupName);

watch(
  () => monitorStore.didChange,
  () => {
    saveUrl();
  },
  { deep: true }
...
async function saveUrl() {
  const data = await monitorStore.getMonitor(groupName.value.toUpperCase());
  if (data && data.length > 0) {
    monitorTypes.value = data;
  } else {
    await monitorStore.updateMonitor(groupName.value.toUpperCase());
    const newData = await monitorStore.getMonitor(
